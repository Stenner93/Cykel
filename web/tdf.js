...
function tdfMiniBar(signals, discRaw, discCo, discKey, stageType) {
  const dk = (discKey || 'DISC').toUpperCase();
  const coLabel = stageType === 'ttt' ? 'holdsnit CO' : 'CO';
  const discTitle = discRaw != null
    ? `${dk}: ${discRaw.toFixed(0)}/100 i felt` + (discCo != null ? ` (${coLabel}: ${discCo.toFixed(0)})` : '')
    : 'Disciplin';
