...
Output:
    web/data/vuelta2026_predictions.json   — per-stage predictions + actuals
    web/data/vuelta2026_scores.json        — score matrix for all riders
"""
...
VUELTA_WINNER_POINTS: dict[str, int] = {
    "sprint":   560_000,
    "mountain": 630_000,
    "hilly":    500_000,
    "tt":       380_000,
...
def _raw(name: str) -> Path:
    VUELTA_RAW_DIR.mkdir(parents=True, exist_ok=True)
    return VUELTA_RAW_DIR / name
...
def main() -> None:
    ...
    if args.no_holdet and PLAYERS_CACHE.exists():
        ...
        print(f"  [cache] {len(riders)} ryttere fra vuelta2026_players.json")
    else:
        print("  Henter spillerliste fra Holdet…")
        try:
            player_by_id, person_by_id = _h.fetch_player_info(VUELTA_GAME_ID, VUELTA_CARTRIDGE)
    ...
    if sched_cache and args.no_holdet:
        ...
        try:
            events, event_info = _h.fetch_schedule(VUELTA_GAME_ID)
    ...
    stage_types_meta = pcs_types_cache.get(VUELTA_PCS_RACE + "_meta", {})
    ...
    if context_path.exists():
        ...
        race_ctx = raw_ctx.get(VUELTA_CARTRIDGE, {})
    ...
    if finished and not args.no_holdet:
        ...
        dns_list = check_dns(riders, VUELTA_PCS_RACE)
    ...
    for _pos, eid in enumerate(events, start=1):
        ...
        if act_cache is None and not args.no_holdet:
            act_cache = fetch_stage_actions(VUELTA_GAME_ID, eid)
        ...
        if cached_sum is None and not args.no_holdet:
            summary_by_pid = fetch_scoring_summary(VUELTA_GAME_ID, eid)
    ...
    for _pos, eid in enumerate(events, start=1):
        ...
        preds = predict_all(
            ...
            profile_score=profile_score,
            winner_pts_override=VUELTA_WINNER_POINTS,
    ...
    scores_path = WEB_DIR / "vuelta2026_scores.json"
    ...
    out_path = WEB_DIR / "vuelta2026_predictions.json"
